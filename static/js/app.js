...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadTemplates() {
        try {
            const response = await fetch('/api/template/list_std');
            ...
            <div class="template-item ${selectedTemplateId === t.id ? 'selected' : ''}" data-id="${t.id}">
                <div class="template-info">
                    ...
                    <div class="template-meta">
                        ${t.sections_count} 个章节 · ${t.paragraphs_count} 个段落 · ${formatTime(t.upload_time)}
                    </div>
                </div>
    ...
    async function uploadTemplate(file) {
        ...
        try {
            const response = await fetch('/api/template/upload_std', {
                method: 'POST',
                body: formData
            });
            const data = await response.json();
    ...
    function selectConvertMode(mode) {
        ...
        if (optionPreset1) optionPreset1.classList.toggle('selected', mode === 'preset_1');
        if (optionPreset2) optionPreset2.classList.toggle('selected', mode === 'preset_2');
        if (optionPreset3) optionPreset3.classList.toggle('selected', mode === 'preset_3');
    ...
    async function analyzeFile(file) {
        ...
        try {
            ...
            try {
                ...
                console.error('JSON解析失败，原始响应:', responseText.substring(0, 500));
                showError('服务器返回了无效的响应格式，请检查后端服务是否正常运行。');
                return;
            }
    ...
    async function convertWithTemplate() {
        ...
        try {
            const response = await fetch('/api/template/' + selectedTemplateId + '/apply', {
                method: 'POST',
                body: formData
            });
    ...
    function showResult(data, usedTemplate = false) {
        ...
        // 优先使用 report_id 进行下载，兼容旧版本使用 output_file
        const reportId = data.report_id || data.output_file;
        downloadBtn.href = '/api/download/' + encodeURIComponent(reportId);
