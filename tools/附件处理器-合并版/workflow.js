...
/**
 * 从文件夹名解析附件索引
 * 支持格式：附件一、附件1、01_附件一、附件一_名称 等
 */
function parseFolderToAttachmentIndex(folderName) {
  // 匹配 "附件X" 格式，X 可以是中文数字或阿拉伯数字
...
/**
 * 解析文件夹结构，自动构建附件映射规则和扁平化图片列表
 * @param {Array} folderData - [{folderName: "附件一", files: [{name, filepath, size}]}]
 * @returns {Object} { images: 扁平化图片数组, ranges: 自动生成的映射规则 }
 */
function parseFolderStructure(folderData) {
  if (!folderData || folderData.length === 0) {
    return { images: [], ranges: [] };
  }

  // 按文件夹名解析附件索引，并排序
  const parsedFolders = folderData
    .map(f => ({
      ...f,
      attIdx: parseFolderToAttachmentIndex(f.folderName)
    }))
    .filter(f => f.attIdx >= 0)
    .sort((a, b) => a.attIdx - b.attIdx);

  if (parsedFolders.length === 0) {
    // 如果没有识别出附件文件夹，把所有图片按顺序放入一个附件
    const allFiles = [];
...
// 5. 生成Markdown
// ============================================================
function generateMarkdown(docInfo, attachments, onProgress) {
  const lines = [];
  ...
  for (let ai = 0; ai < attachments.length; ai++) {
    ...
    onProgress?.(ai + 1, total, `生成附件${att.indexChinese}：${att.name}`);
  }
